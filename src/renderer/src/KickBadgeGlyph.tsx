 ...
 * so they are drawn here from Kick's own artwork. Every gradient and clip id is
 * suffixed per instance, because many messages carry the same badge and shared
 * ids would collide, and a stable id would break when the message that owns it
 * scrolls out and unmounts.
 */
...
export function KickBadgeGlyph({ glyph, title }: KickBadgeGlyphProps) {
  ...
  if (glyph === "verified") {
    const grad = `kbv-grad-${uid}`;
    const clip = `kbv-clip-${uid}`;
    return (
      <svg
        aria-label={title}
  ...
  if (glyph === "sub_gifter") {
    ...
    return (
      ...
      >
        <g clipPath={`url(#${clip})`}>
          <path d="M22.34 9.5L26 4H18L16 7L14 4H6L9.66 9.5H4V15.1H28V9.5H22.34Z" fill="#2EFAD1" />
          <path d="M26.08 19.1001H5.90002V28.5001H26.08V19.1001Z" fill="#2EFAD1" />
          <path d="M26.08 15.1001H5.90002V19.1001H26.08V15.1001Z" fill="#00A18D" />
        </g>
        <defs>
          <clipPath id={clip}>
            <rect fill="#fff" height="24.5" transform="translate(4 4)" width="24" />
          </clipPath>
        </defs>
      </svg>
    );
  ...
  // moderator: a rounded square with the Kick mark cut out, layered gradients.
  const gA = `kbm-a-${uid}`;
  const gB = `kbm-b-${uid}`;
  const gC = `kbm-c-${uid}`;
  const clip = `kbm-clip-${uid}`;
  const modPath =
  ...
  return (
    ...
    >
      <g clipPath={`url(#${clip})`}>
        <path d={modPath} fill={`url(#${gA})`} />
        <path d={modPath} fill={`url(#${gB})`} />
        <path d={modPath} fill={`url(#${gC})`} />
      </g>
      <defs>
        <linearGradient gradientUnits="userSpaceOnUse" id={gA} x1="18.8102" x2="2.88536" y1="-12.7222" y2="39.1063">
          <stop stopColor="#FF6A4A" />
          <stop offset="1" stopColor="#C70C00" />
        </linearGradient>
        <linearGradient gradientUnits="userSpaceOnUse" id={gB} x1="15.7467" x2="16.321" y1="-4.75575" y2="39.0672">
          <stop stopColor="#FFC900" />
          <stop offset="0.99" stopColor="#FF9500" />
        </linearGradient>
        <linearGradient gradientUnits="userSpaceOnUse" id={gC} x1="-14.9543" x2="32.0001" y1="46.9544" y2="-0.000509222">
